...
Active plan: `.jons-plan/active-plan`
...
| `planning_panel` | bool | No | Enable 2-agent planning panel (Opus + Codex). Requires `use_tasks` and a `codex-rescue` executor task. |
...
| `executor` | Execution method: `task-tool` (default), `codex-rescue`, `codex-review`, or `codex-adversarial-review` |
